...
import '../utils/test_result_manager.dart'; // Ensure this path matches your folder structure
...
class _ADHDTestScreenState extends State<ADHDTestScreen> {
  // ASRS v1.1 Clinical Screening Questions
  final List<Map<String, dynamic>> questions = [
    {
      ...
      'options': [{'option': 'Never', 'points': 0}, {'option': 'Rarely', 'points': 1}, {'option': 'Sometimes', 'points': 2}, {'option': 'Often', 'points': 3}, {'option': 'Very Often', 'points': 4}],
    },
    {
  ...
  void checkIfTestTaken() async {
    ...
    if (user != null) {
      try {
        ...
        if (docSnapshot.exists && docSnapshot.data()!.containsKey('ADHDTimestamp')) {
          Timestamp lastTimestamp = docSnapshot.data()!['ADHDTimestamp'];
    ...
    return Scaffold(
      ...
      floatingActionButton: (!testTaken && selectedOptions.length == questions.length)
          ? FloatingActionButton.extended(
              ...
              label: const Text("Generate Insight", style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        children: [
          Row(
            children: [
              ...
              const Text("ADHD Analysis", style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
            ],
          ),
...
void _submitTest() async {
  ...
  // 1. Get the comprehensive feedback from your new Manager
  final feedback = TestResultManager.getFeedback('ADHD', totalScore);

  final user = FirebaseAuth.instance.currentUser;
  if (user != null) {
    await FirebaseFirestore.instance.collection('Users').doc(user.uid).set({
      'ADHDTestScore': totalScore,
      'ADHDDiagnosis': feedback['status'], // Use the status from your manager
      'ADHDTimestamp': FieldValue.serverTimestamp(),
    }, SetOptions(merge: true));

    // 2. Show the enhanced Result Sheet instead of the old Dialog
    _showEnhancedResultSheet(totalScore, feedback);
  }
}

  String _calculateDiagnosis(int score) {
    if (score <= 5) return 'Low likelihood of ADHD';
    if (score <= 13) return 'Moderate indicators of ADHD';
    return 'High likelihood of ADHD (Clinical follow-up recommended)';
  }
